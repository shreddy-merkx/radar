...
2. `build/fetch.mjs` ruft alle 199 Feeds ab (serverseitig, kein CORS),
   entdoppelt, siebt nach Alter, und schreibt `app/data/digest.json`
   (350 Meldungen, ~150 KB).
3. Der Ablauf veröffentlicht `app/` direkt auf GitHub Pages. **Es wird nichts
...
| Datei | Zweck |
|---|---|
| `app/index.html` | Gerüst, PWA-Metaangaben |
| `app/app.js` | Anzeige, Zettel, Einstellungen, Ziehen zum Aktualisieren |
| `app/styles.css` | Farben unverändert aus der alten App (`theme.ts`) |
| `app/sw.js` | Offline: Gerüst aus dem Zwischenspeicher, Nachrichten aus dem Netz |
| `app/lib/sources.js` | **199 Quellen, 9 Rubriken. Hier wächst die App.** |
| `app/lib/localFilter.js` | Bewerten, mischen, kürzen |
| `app/lib/events.js` | Terminkalender, 55 Termine |
| `build/fetch.mjs` | Der Einsammler |
| `build/rss.mjs` | Abruf und XML-Auswertung |

`app/lib/*` wurde per `node:module.stripTypeScriptTypes` aus dem alten
TypeScript erzeugt — inhaltlich Zeile für Zeile dasselbe.
...
2. **`build/fetch.mjs` bricht ab, wenn unter 25 % der Quellen antworten.**
...
10. **Beim Ändern von `app/sw.js` oder dem Gerüst die Zahl in `CACHE`
...
- **Die npm-Registry und alle Feeds sind aus der Sitzungsumgebung zeitweise
  ...
  aus, und `build/fetch.mjs` lässt sich mit einer `fetch`-Attrappe komplett
